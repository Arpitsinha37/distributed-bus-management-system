...
async function runTests() {
  ...
  const tripId = search.body[0].tripId;
  ...
  const payload = {
    ...
    passengers: [
      { name: "Test User", seatNumber: "A1" }
    ]
  };

  const hold = await request('POST', `/bookings/hold`, { 'X-Site-Id': 'pokhara-travels' }, payload);
  console.log("Hold response:", hold.status, hold.body.id || hold.body.message);
  ...
  if (hold.status === 201) {
    ...
    console.log("Pay success:", payOk.status, payOk.body.status || payOk.body.message);
