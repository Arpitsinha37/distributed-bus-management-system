...
export class PaymentService {
    ...
    async recoverPendingPayments() {
        ...
        const pending = await this.prisma.nrtPayment.findMany({
            where: {
                status: 'pending',
    ...
    async retryFailedBusPortalConfirmations() {
        try {
            const failedConfirmations = await this.prisma.nrtPayment.findMany({
                where: {
                    status: 'completed',
                    isFinalized: true,
            ...
            for (const payment of failedConfirmations) {
                try {
                    ...
                    await this.prisma.nrtPayment.update({
                        where: { id: payment.id },
                        data: {
                            lastError: null,
                            gatewayResponse: {
    ...
    async autoExpireStalePendingPayments() {
        ...
        const stalePending = await this.prisma.nrtPayment.findMany({
            where: {
                status: 'pending',
                ...
                if (payment.method === 'esewa' && payment.transactionId) {
                    try {
                        ...
                        if (verifyResponse?.data?.status === 'COMPLETE') {
                            const updated = await this.prisma.nrtPayment.update({
                                where: { id: payment.id },
    ...
    async initiatePayment(dto: {
        ...
        const existingPending = await this.prisma.nrtPayment.findMany({
        ...
        if (existingPending.length > 0) {
            ...
            await this.prisma.nrtPayment.updateMany({
                where: { ticketNo: dto.ticketNo, status: 'pending' },
                data: {
                    status: 'failed',
    ...
    async verifyCardPayment(paymentId: string, transactionId?: string) {
        const payment = await this.prisma.nrtPayment.findUnique({ where: { id: paymentId } });
        ...
        const updatedPayment = await this.prisma.nrtPayment.update({
            where: { id: paymentId },
            data: {
                status: 'processing',
    ...
    async syncPaymentStatus(paymentId: string) {
        ...
        if (['khalti', 'khalti_mobilebanking', 'khalti_ebanking', 'khalti_connectips'].includes(payment.method)) {
             ...
             return { 
                 ...
                 payment: await this.prisma.nrtPayment.findUnique({ where: { id: paymentId } }) 
             };
        }
    ...
    async syncManualBookingFromPortal(ticketNo: string): Promise<any | null> {
        try {
            ...
            const existing = await this.prisma.nrtPayment.findFirst({ where: { ticketNo } });
            ...
            const saved = await this.prisma.nrtPayment.create({
                data: {
    ...
    async findByPhone(phone: string) {
        const payments = await this.prisma.nrtPayment.findMany({
    ...
    async findAll(page = 1, limit = 20, method?: string, status?: string) {
        ...
        try {
            ...
            await this.prisma.nrtPayment.deleteMany({
                where: {
                    status: 'failed',
    ...
    async getStats(filters?: { startDate?: string; endDate?: string; method?: string; status?: string }) {
        ...
        const [
            ...
            this.prisma.nrtPayment.count({ where }),
            this.prisma.nrtPayment.count({ where: { ...where, status: 'completed' } }),
            this.prisma.nrtPayment.count({ where: { ...where, status: { in: ['pending', 'processing'] } } }),
            this.prisma.nrtPayment.count({ where: { ...where, status: 'failed' } }),
            this.prisma.nrtPayment.count({ where: { ...where, method: 'esewa' } }),
            this.prisma.nrtPayment.count({ where: { ...where, method: { startsWith: 'khalti' } } }),
            this.prisma.nrtPayment.count({ where: { ...where, method: 'cash_on_bus' } }),
    ...
    async getAnalytics(period: string = '30d') {
        ...
        const couponPayments = await this.prisma.nrtPayment.findMany({
    ...
    private async _finalizePaymentSuccess(payment: any, method: string, transactionId: string) {
        ...
        // ═══════════════════════════════════════════════════════════════
        const { count } = await this.prisma.nrtPayment.updateMany({
