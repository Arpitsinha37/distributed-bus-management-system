...
export default function PaymentsPage() {
    ...
    // Auto-sync gateway logs when opening a payment with missing gatewayResponse
    useEffect(() => {
        if (selectedPayment && !selectedPayment.gatewayResponse && selectedPayment.status === 'completed' && !syncing) {
            const autoSync = async () => {
                setSyncing(true);
                try {
                    const res = await apiPost<{ message: string; payment: any }>(`/payments/${selectedPayment.id}/sync`, {}, accessToken!);
                    if (res.payment) {
    ...
    const handleFetchLogs = async () => {
        ...
        try {
            ...
            alert(err.message || 'Failed to fetch logs from gateway');
        }
        setSyncing(false);
    };
                        ...
                        <div className="p-6 overflow-y-auto max-h-[70vh]">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                ...
                                <section>
                                    ...
                                    {syncing ? (
                                        <div className="flex items-center gap-2 bg-black text-yellow-400 p-3 rounded-lg text-[10px] font-mono">
                                        ...
                                        <div className="bg-black rounded-lg p-3 space-y-2">
                                            <pre className="text-[9px] text-red-400 font-mono">{JSON.stringify({ message: "No data logs available" }, null, 2)}</pre>
                                            <button
                                                onClick={handleFetchLogs}
                                                className="flex items-center gap-1.5 px-3 py-1.5 bg-green-600 hover:bg-green-500 text-white text-[10px] font-bold rounded-md transition-colors"
                                            >
                                                <RefreshCw className="w-3 h-3" /> Fetch Logs from Gateway
                                            </button>
                                        </div>
                                    )}
                                </section>
