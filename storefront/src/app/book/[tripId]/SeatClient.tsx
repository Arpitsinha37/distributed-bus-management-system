...
import { ArrowLeft, Info, Snowflake, Wifi, Droplets, BedDouble, Tag, ChevronRight, Bus } from 'lucide-react';
...
export default function SeatClient({ trip }: { trip: TripDetail }) {
  ...
  const handleProceed = () => {
    ...
    // In nextjs app, holding happens locally or directly in API on Passenger Details page,
    // so we just route to details.
  ...
  for (let r = 0; r < rows; r++) {
    for (let c = 1; c <= columns; c++) {
      ...
      mappedSeats.push({
        ...
        bookingStatus,
        gender: 'none', // Not tracking gender in DB yet
      });
    }
  }

  const price = Number(trip.fare);
  const cashback = 0; // If you have cashback field in trip, set it here
        ...
        <div className="w-full mx-auto flex flex-col md:flex-row gap-6 md:gap-8">
            ...
            <div className="w-full md:w-1/3 flex flex-col gap-6 order-2 md:order-1">
                ...
                <div className="rounded-xl border shadow-sm p-6 relative overflow-hidden group" style={{ background: C.surface, borderColor: C.surfaceVariant }}>
                    ...
                    <h2 className="text-xl font-semibold mb-1" style={{ color: C.onSurface }}>{trip.bus?.operator}</h2>
